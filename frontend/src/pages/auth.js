...
export default function Auth({ defaultTab = "login", onSuccess, onBack }) {
  ...
  const handleLogin = () => {
    if (!loginEmail || !loginPassword) { showToast("Please fill in all fields", "error"); return; }
    if (!/\S+@\S+\.\S+/.test(loginEmail)) { showToast("Invalid email address", "error"); return; }
    if (loginPassword.length < 6) { showToast("Password must be at least 6 characters", "error"); return; }
    // TODO: call login API
    showToast("Login successful! Welcome back.");
    setTimeout(() => onSuccess?.({ email: loginEmail, name: loginEmail.split("@")[0] }), 1000);
  };

  const handleRegister = () => {
    if (!regName || !regEmail || !regPhone || !regPassword || !regConfirm) { showToast("Please fill in all fields", "error"); return; }
    if (!/\S+@\S+\.\S+/.test(regEmail)) { showToast("Invalid email address", "error"); return; }
    if (regPassword.length < 6) { showToast("Password must be at least 6 characters", "error"); return; }
    if (regPassword !== regConfirm) { showToast("Passwords do not match", "error"); return; }
  ...
  const handleForgot = () => {
    if (!forgotEmail || !/\S+@\S+\.\S+/.test(forgotEmail)) { showToast("Please enter a valid email", "error"); return; }
      ...
      {showForgot && (
        <div style={s.overlay} onClick={() => setShowForgot(false)}>
          <div style={s.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <p style={s.modalSub}>Enter your email and we'll send you a link to reset your password.</p>
            <label style={s.label}>Email Address</label>
            <input style={s.input} type="email" placeholder="your@email.com"
              value={forgotEmail} onChange={(e) => setForgotEmail(e.target.value)}
              onFocus={focus} onBlur={blur}ss
              onKeyDown={(e) => e.key === "Enter" && handleForgot()} />
            <div style={s.modalRow}>
              <button style={s.btnGhost} onClick={() => setShowForgot(false)}>Cancel</button>
              <button style={{ ...s.btnPrimary, marginTop: 0, flex: 2 }} onClick={handleForgot}
                onMouseEnter={(e) => (e.currentTarget.style.background = "#7A5C35")}
                onMouseLeave={(e) => (e.currentTarget.style.background = "#1C1A16")}
